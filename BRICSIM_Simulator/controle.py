def aplicar_controle_PID(canvas, dt=0.1):
    ...
    def _arb_mv(tag: str, mv_calc: float, vg_obj=None) -> float:
        ...
        if mv_iq_val is not _S:
            try:
                ...
                mv_val = mv_calc
            if mv_val < 0.0: mv_val = 0.0
            if mv_val > 100.0: mv_val = 100.0
            return mv_val
    ...
    items = [it for it in getattr(canvas, "variaveis", [])
    ...
    def _conf(item):
        ...
        return conf, pv_tag, sp_tag, mv_tag, pv_min, pv_max, Kp, Ki, Kd, sign, sp_tracking, mv_write_tag
    ...
    for item in items:
        modo = vg.get(f"{item.tag}.modo", "MAN")
        conf, pv_tag, sp_tag, mv_tag, pv_min, pv_max, Kp, Ki, Kd, sign, _spt, mv_write_tag = _conf(item)
        ...
        if modo == "AUT":
            ...
            fonte = conf.get("fonte_cascata", "") or item.controle.get("fonte_cascata", sp_tag)
            sp = vg.get(fonte, pv)
        ...
        if (mv_pid != mv_raw) and ((mv_pid == 100 and erro > 0) or (mv_pid == 0 and erro < 0)):
        ...
        if mv_write_tag:
            ...
            if vg.get(f"{slave_tag}.modo", "MAN") == "CAS":
                ...
                if s_pvmin > s_pvmax: s_pvmin, s_pvmax = s_pvmax, s_pvmin
    ...
    for item in items:
        modo = vg.get(f"{item.tag}.modo", "MAN")
        conf, pv_tag, sp_tag, mv_tag, pv_min, pv_max, Kp, Ki, Kd, sign, sp_tracking, mv_write_tag = _conf(item)
