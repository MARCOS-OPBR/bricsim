...
class TrendWindow(QtWidgets.QDialog):
    ...
    """
    def __init__(self, parent=None, titulo: str = "Tendências", max_points: int = 600, dt_ms: int = 500):
        ...
        self.in_line.setPlaceholderText('Ex.: TI001|#ffcc00|°C  ou  LC001|red|m  (deixe cor/escala vazios se quiser)')
        ...
        side = QtWidgets.QWidget(); side.setLayout(right_col)
        ...
        self.spin_history = QtWidgets.QSpinBox(); self.spin_history.setRange(60, 20000); self.spin_history.setValue(self._max_points)
    ...
    def _ensure_axis(self, escala: str):
        ...
        layout_obj = plot_item.layout if not callable(getattr(plot_item, "layout", None)) else plot_item.layout()
    ...
    def _add_pen(self, tag: str, color: str = '', escala: str = ''):
        ...
        self.list_pens.addItem(f"{tagn} | {color or 'auto'} | {item['axis'] or 'primária'}")
        self._pens.append(item)
        self.list_pens.addItem(f"{tagn} | {color or 'auto'} | {item['axis'] or 'primária'}")
